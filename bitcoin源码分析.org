...
* 必读--如何阅读org文档
  - 如果打开org文档 :: 请使用Emacs编辑器打开org文档！Windows环境的[[http://mirrors.ustc.edu.cn/gnu/emacs/windows/][下载链接]],Linux下直接使用apt-get或者yum直接安装emacs即可。
  ...
  - 如何生成HTML文档 :: 菜单栏里选择Org->Export/Publish(C-c C-e),C代表Ctrl按键,先按住Ctrl+c，再按住Ctrl+e，也会调用导出HTML的选项
  - 配置bitcoin源代码目录 :: 修改，以便迅速在Emacs编辑器中打开bitcoin源代码。
  - 如何链接到源代码 :: 比如file:~/.bitcoin ，将光标移动到file开头的链接上，按住Alt+x,接着输入org-open-at-point回车，就会自动跳转到源代码了，如果是目录，就会打开目录。
  ...
  : git clone --branch v0.8.2 https://github.com/bitcoin/bitcoin.git
    注意将bitcoin源代码目录放在~目录下，Windows下为export %AppData%的目录下
